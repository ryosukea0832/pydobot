...
class Dobot:
    ...
    def _set_motor(self, motor,speed):
        STEP = 360.0 / 1.8 * 10.0 * 16.0
        MM = 3.141592653589 * 36.0
        vle = 0
        msg = Message()
        msg.id = CommunicationProtocolIDs.SET_MOTOR
        ...
        if speed is 0x00:
            ...
            msg.params.extend(bytearray([0x01]))
            vle = float(30) * (STEP / MM)
            print("pydobot:",vle)
            
      
        #print("pydobot:",struct.pack('i', vle))
        msg.params.extend(bytearray([0x21]))
        msg.params.extend(bytearray([0x28]))
        msg.params.extend(bytearray([0x00]))
        msg.params.extend(bytearray([0x00]))
    ...
    def _set_sensor(self):
        msg = Message()
        msg.id = CommunicationProtocolIDs.SET_GET_IRSWITCH
        msg.ctrl = ControlValues.ONE
        msg.params = bytearray([])
        msg.params.extend(bytearray([0x01]))
        msg.params.extend(bytearray([0x02]))
        msg.params.extend(bytearray([0x01]))
        return self._send_command(msg)    
    ...
    def motor(self, motor,speed):
        self._set_motor(motor,speed)
        
    def set_sensor(self):
        self._set_sensor()
        
    def get_sensor(self):        
        response = self._get_sensor()
        
	#if self.old_sensor == response: sensor = 0x01
	#else: sensor = 0x00
	
        self.old_sensor = response
        return response
